...
long long solve(int n, int a[20][20]) {
	long long max = 0;
	for (int i = 0; i < 20; i++){
		for (int j = 0; j < 20-n; j++){
			long long temp1 = 1, temp2 = 1;
			for (int k = 0; k < n; k++){		
				temp1 *= (long long)(a[i][j+k]);
				temp2 *= (long long)(a[j+k][i]);
			}
	...
	for (int i = 0; i < 20-n; i++){
		for (int j = 0; j < 20-n; j++){
			long long temp1 = 1, temp2 = 1;
			for (int k = 0; k < n; k++){
				temp1 *= (long long)(a[i+k][j+k]);
				temp2 *= (long long)(a[19-i-k][j+k]); 
			}
			if (temp1 > max && temp1 > temp2)
				max = temp1;
			else if (temp2 > max && temp2 > temp1)
